...
func commonServers() (list []string) {
	err := gophig.GetConfComplex("assets/common_servers.json", gophig.JSONMarshaler{}, &list)
	if err != nil {
		fmt.Println(err)
	}
	return list
...
func blockingServers() (list []string) {
	err := gophig.GetConfComplex("assets/blocking_servers.json", gophig.JSONMarshaler{}, &list)
	if err != nil {
		fmt.Println(err)
	}
	return list
...
func init() {
	go func() {
		for {
			common := commonServers()
			for _, server := range common {
			...
			<-time.After(time.Duration(60/len(common)) * time.Minute)
		}
	}()
}
...
func (a *App) download(w http.ResponseWriter, r *http.Request) {
	...
	for _, server := range blockingServers() {
